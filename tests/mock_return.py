...
class MockReturn:
    ...
    def generate_reset(self):
        ...
        self.block = cw_dcp.DCPBlockRequest(opt, subopt, len(block_content), block_content)
        return self.compose_response()

    def compose_response(self):
        dcp = cw_dcp.dcp_header(self.frame_id, self.service_id, self.service_type, self.xid, 0x0000, len(self.block), payload=self.block)
